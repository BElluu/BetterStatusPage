...
```bash
export BSP_IMAGE=ghcr.io/belluu/better-status-page:0.1.2
docker compose up -d
```
...
|----------|----------|-------------|
| `BSP_IMAGE` | Docker Compose | Container image used by `docker-compose.yml`, for example `ghcr.io/belluu/better-status-page:0.1.2`. |
