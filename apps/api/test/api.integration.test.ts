...
before(async () => {
  ...
  const inserted = await db.insert(monitors).values({
    ...
    config: '{}',
    currentStatus: 'down',
    webhookToken: 'integration-token',
    tags: '[]',
    createdAt: now,
    updatedAt: now,
  }).returning()
...
describe('public API integration', () => {
  ...
  it('aggregates monitor uptime and response history', async () => {
    ...
    assert.equal(clamped.json().hours, 168)
    assert.equal(clamped.json().buckets.length, 10)
  ...
  it('accepts a valid heartbeat and rejects an invalid token', async () => {
    ...
    const heartbeat = await app.inject({ method: 'POST', url: '/api/v1/hook/integration-token' })
