...
function alignBrandingDefaultsWithLightMode(): void {
  const migrationName = 'branding-defaults-match-light-mode-v1'
  sqlite.exec('BEGIN IMMEDIATE')
  try {
    const applied = sqlite.prepare('SELECT 1 FROM schema_migrations WHERE name = ?').get(migrationName)
    if (!applied) {
      sqlite.exec(`
        UPDATE branding SET
      ...
      `)
      sqlite.prepare('INSERT INTO schema_migrations (name, applied_at) VALUES (?, ?)').run(migrationName, Date.now())
    }
    sqlite.exec('COMMIT')
  } catch (error) {
